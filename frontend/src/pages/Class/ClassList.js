...
function ClassList() {
  ...
  const StudentManagementModal = ({ show, onClose, classId }) => {
    ...
    const onSearchStudent = async () => {
      ...
      try {
        const response = await axios.get(
          `${STUDENT_API}/search/${studentSearchKeyword}`
        );
    ...
    const unenrollStudent = async (studentId) => {
      ...
      try {
        // Send the array of studentIds in the request body
        await axios.delete(`${CLASS_API}/${classId}/student`, {
          data: studentIdList,
        });
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-sm">
                    {student.name} - {student.gender}, Age: {student.age}
                  </span>
                  <span className="px-2 rounded-full text-white bg-green-500 text-sm font-sm">
                    Enrolled
                  </span>
                </div>
                <Button
      ...
      <div className="w-full h-full p-10 py-7">
        <div className="mb-5 flex items-center justify-between">
          <MdOutlineKeyboardBackspace
            ...
            onClick={() => navigate("/")}
          />
          <h2 className="font-semibold text-2xl">Class Management</h2>
          <Button onClick={openCreateClassForm} color="primary">
            + Create Class
          </Button>
        </div>

        <div className="overflow-x-auto bg-white bg-opacity-60 rounded-md">
                    ...
                    <td>
                      <div className="flex items-center justify-start space-x-2">
                        ...
                        <Button
                          ...
                          onClick={() => openStudentManagement(classData.id)}
                          size="xs"
                        >
                          Manage Students
                        </Button>
                        <Button
                          className="text-white bg-green-500 hover:bg-green-600"
                          onClick={() => openTeacherAssign(classData.id)}
                          size="xs"
                        >
                          Teacher Assign
                        </Button>
