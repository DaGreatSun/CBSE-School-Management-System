...
import { TEACHER_API } from "../../utils/api";
...
function TeacherList() {
    ...
    const [qualification, setQualification] = useState("doctoral degree"); //Master degree, Bachelor Degree
    const [position, setPosition] = useState("class teacher"); //Head of Department, Counseling Teacher, Coordinator
    ...
    const columns = [
        ...
        { field: "createdDate", text: "Teacher Created Date" },
    ...
    const positionSelection = [
        ...
        {
            text: "Counseling Teacher",
            value: "counseling teacher",
        },
        {
    ...
    async function submitCreate() {
        ...
        try {
            const res = await axios.post(TEACHER_API, data);
            ...
            if (res.status === HTTP_STATUS.CREATED) {
                ...
                getTeacherList();
                reset();
                toast.success("Created new teacher successfully!");
            }
        } catch (e) {
            toast.error("Error in creating teacher. Please try again.");
    ...
    async function submitUpdate() {
        ...
        try {
            const res = await axios.put(TEACHER_API, data);
            ...
            toast.error("Error in updating teacher. Please try again.");
            toastValidationError(e.response.data);
        }
    }
    ...
    async function toDelete(id) {
        try {
            const res = await axios.delete(TEACHER_API + "/" + id);
    ...
    async function onSearch() {
        ...
        if (search !== "") {
            try {
                const res = await axios.get(TEACHER_API + "/search/" + search);
                const data = res.data;
        ...
        return (
            <div className="w-full h-full p-10 py-7">
                <div className="mb-5 flex items-center">
                    ...
                    />
                    <div className="font-semibold text-2xl">All Teachers</div>
                </div>
                ...
                <div className="mt-10 px-5 min-w-min">
                    <MyTable cardView={isCardView} columns={columns} data={teacherList} recordType="teacher" />
                </div>
                ...
                <MyForm
                    ...
                    title={onCreateOrEdit ? "Create Teacher" : "Edit Teacher"}
